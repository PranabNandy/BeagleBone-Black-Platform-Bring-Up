...
- `udev` program listens to the `uevents` generated by hot plug events or kernel modules. When `udev` receives the `uevents`, it scans the subdirectories of `/sys/class/` (i.e `/sys/class/pcd_class/pcd/`) looking for the **"dev"** files to create device files.
